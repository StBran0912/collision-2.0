...
export interface MoverBox extends Mover {
    me: BoxProps
}

export interface MoverBall extends Mover {
    me: BallProps
}
...
export function createBox(posx: number, posy:number, w:number, h: number): MoverBox {
    ...
    function applyForce(force: lb2d.Vector, angForce: number) {
    ...
    function resetPos(v: lb2d.Vector) {
        me.location.add(v);
        me.vertices[0].add(v);
        me.vertices[1].add(v);
        me.vertices[2].add(v);
...
function checkCollisionBoxes(a: MoverBox, b:MoverBox) {
...
function checkCollisionBalls(a: MoverBall, b:MoverBall) {
//Distanz ermitteln
...
function checkCollisionBallBoxes(ball:MoverBall, box:MoverBox) {
    for (let j = 0; j < 4; j++) {
...
function resolveCollisionBallBoxes(ball: MoverBall, box: MoverBox, cp: lb2d.Vector, normal:lb2d.Vector) {
...
export function checkCollision(a: MoverBall|MoverBox, b: MoverBall|MoverBox) {
    if (a.me.typ == "Ball") {
        if (b.me.typ == "Ball") {
            // @ts-ignore
